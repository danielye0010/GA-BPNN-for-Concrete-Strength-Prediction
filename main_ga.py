...
GA_OPTIMIZATION_PATH = OUTPUTS_DIR + "/" + "ga_optimization.csv"  # GA优化路径
...
def load_data():
    # 加载数据
    data = pd.read_csv("data.csv", header=None, names=["x1", "x2", "x3", "x4", "y"], usecols=[0, 1, 2, 3, 4])  # 读取数据
...
def train(
    ...
    for epoch in range(1, epochs + 1):
        ...
        # 如果测试的 R^2 值超过 0.92，打印信息并中断循环
        if test_r2 > 0.92:
            print(f"Reached R^2 of {test_r2} in epoch {epoch}. Stopping training.")
            break

        if epoch == epochs:
            print(f"best test r2: {max(test_r2_records)}, training finished!")
...
def train_run(parameter):
    batch_size, lr, epochs, hidden1_size, hidden2_size, dropout = parameter
...
def ga_optimization():
    ga = GA(
        ...
        max_iter=20,
        prob_mut=0.001,
        lb=[8, 0.00001, 100, 16, 16, 0.1],
        ub=[128, 0.01, 1000, 128, 128, 0.5],
        precision=[1, 1e-5, 1, 1, 1, 1e-1],
    )  # 定义GA算法
    ga.run()  # 运行算法

    print("best x:", ga.best_x.tolist())
    print("best y:", ga.best_y.tolist())
    print("generation best x:", np.array(ga.generation_best_X).tolist())
    print("generation best y:", ga.generation_best_Y)
    print("cumulative minimum y:", pd.DataFrame(ga.all_history_Y).min(axis=1).cummin().tolist())

    pd.DataFrame(
        {
            "parameter": [
                {p: x for p, x in zip(["batch_size", "lr", "epochs", "hidden1_size", "hidden2_size", "dropout"], item)}
                for item in np.array(ga.generation_best_X).tolist()
            ],
            "y": [-1 * y for y in ga.generation_best_Y],
        }
    ).to_csv(
        GA_OPTIMIZATION_PATH, index=False
    )  # 保存GA优化数据

    return ga.best_x.tolist()


if __name__ == "__main__":
    batch_size, lr, epochs, hidden1_size, hidden2_size, dropout = ga_optimization()  # GA 参数优化
    train_run([batch_size, lr, epochs, hidden1_size, hidden2_size, dropout])  # 训练
    test_run([batch_size, lr, epochs, hidden1_size, hidden2_size, dropout])  # 测试